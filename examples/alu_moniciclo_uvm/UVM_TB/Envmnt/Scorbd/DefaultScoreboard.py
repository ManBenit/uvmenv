            ...
            if not success_dut or not success_rmod:
                ...
                if self.enable_scoreboarding:  
                    try:
                        ...
                        ## Save conditions
                        condition_zerof = response_dut.ex_zerof_o == response_rmod.ex_zerof_o
                        condition_data = response_dut.ex_data_o == response_rmod.ex_data_o

                        # Make assertions
                        assert condition_zerof, f'TEST ERROR zero flag dut({response_dut.ex_zerof_o.integer}), rmod({response_rmod.ex_zerof_o})'
                        assert condition_data, f'TEST ERROR result dut({response_dut.ex_data_o.signed_integer}), rmod({response_rmod.ex_data_o})'
