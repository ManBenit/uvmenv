...
class DefaultBfmImpl(BFM):
    ...
    async def set(self, *, clk,reset):
        ...
        #self.dut.clk.value = clk
        #self.dut.reset.value = reset
    ...
    async def get(self):
        ins = {
            'clk': self.dut.clk.value,
            'reset': self.dut.reset.value
        }

        outs = {
            'count': self.dut.count.value
        }
    ...
    async def init(self):
        ...
        self.clock = Clock(self.dut.clk, CLOCK_CYCLES, units='ns')  
