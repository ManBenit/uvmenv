...
class DefaultScoreboard(uvm_scoreboard):
    ...
    def check_phase(self):
        ...
        while self.dut_result_get_port.can_get() and self.refmodel_result_get_port.can_get():
            ...
            if not success_dut or not success_rmod:
                ...
                if request_dut.rst_ni == 1 and self.enable_scoreboarding == False:
                ...
                if self.enable_scoreboarding:  
                    try:
                        ...
                        if request_dut.wr_i.integer == 1:
                            #report.write(f'[WR] Entrada reporte den {response_dut.den.integer}', self, pyuvm.INFO)
                            #report.write(f'[WR] Escrito: {request_dut.data_i.integer}', self, pyuvm.INFO)
                            assert response_dut.den.integer == 0, f'[WR] Mismatch "den"'

                        if response_dut.full.integer == 1:
                            #report.write('FIFO llena', self, pyuvm.INFO)
                            assert response_rmod.full, f'Mismatch full dut({hex(response_dut.full)}), refmodel({hex(response_rmod.full)})'
                        
                        if request_dut.rd_i.integer == 1:
                            #report.write(f'[RD] Entrada reporte data {response_dut.data_o.integer}', self, pyuvm.INFO)
                            assert response_dut.data_o.integer == response_rmod.data_o, f'Mismatch data dut({hex(response_dut.data_o)}), refmodel({hex(response_rmod.data_o)})'
                            
                        if response_dut.empty.integer == 1:
                            #report.write('FIFO vacía', self, pyuvm.INFO)
                            assert response_rmod.empty, f'Mismatch empty dut({hex(response_dut.empty)}), refmodel({hex(response_rmod.empty)})'
    ...
    def write(self, t):
        if(isinstance(t, uvm_sequence_item)):
            ...
            if self.tr.get_transaction().request.rst_ni == 1:
