...
class ScbDefault(uvm_scoreboard):
    ...
    def check_phase(self):
        ...
        for prefmod, dutin, dutout in zip(self.pending_rmod, self.sync_dut_ins, self.sync_dut_outs):
            ...
            #     print(signame, dutin.get(signame) , getattr(prefmod, signame))
            ...
            if dutin.get('wr_i') == 1 and dutin.get("den") is not None:
                assert dutin.get('den') == 0, f'Missmatch den: DUT({dutin.get("den")}) | RefModel({getattr(prefmod, "den")})'
            
            if dutin.get('rd_i') == 1 and dutin.get('command_o') is not None and dutin.get('data_o') is not None and dutin.get('address_o') is not None:
                assert dutin.get('command_o') == 0, f'Missmatch command_o: DUT({dutin.get("command_o")}) | RefModel({getattr(prefmod, "command_o")})'
                assert dutin.get('data_o')    == 0, f'Missmatch data_o: DUT({dutin.get("data_o")}) | RefModel({getattr(prefmod, "data_o")})'
                assert dutin.get('address_o') == 0, f'Missmatch address_o: DUT({dutin.get("address_o")}) | RefModel({getattr(prefmod, "address_o")})'
