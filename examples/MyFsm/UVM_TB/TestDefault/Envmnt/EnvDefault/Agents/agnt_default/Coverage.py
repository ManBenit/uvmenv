...
class Coverage(uvm_component):
    ...
    @CoverPoint('myfsm.i_clk',
        xf=lambda tr: tr.i_clk,
        bins = [
            i for i in range(2)
        ]
    )
    @CoverPoint('myfsm.i_rst_n',
        xf=lambda tr: tr.i_rst_n,
        bins = [
            i for i in range(2)
        ]
    )
    @CoverPoint('myfsm.i_x',
        xf=lambda tr: tr.i_x,
        bins = [
            i for i in range(2)
        ]
    )
    @CoverPoint('myfsm.o_y',
        xf=lambda tr: tr.o_y,
        bins = [
            i for i in range(2)
        ]
    )
    @CoverPoint('myfsm.o_state',
        xf=lambda tr: tr.o_state,
        bins = [
            i for i in range(4)
        ]
    )
