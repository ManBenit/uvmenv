...
class ALUScoreboard(uvm_scoreboard):
    ...
    def check_phase(self):
        ...
        while self.dut_result_get_port.can_get() and self.refmodel_result_get_port.can_get():
            ...
            if not success_dut or not success_rmod:
                ...
                if request_dut.reset == 0 and self.enable_scoreboarding == False:
                ...
                if self.enable_scoreboarding:  
                    try:
                        ...
                        if(response_dut.result.signed_integer < 0):
                            response_dut.result=response_dut.result.signed_integer
                        ...
                        assert cond_result, f'TEST FAILED result dut({response_dut.result.integer}), rmod({response_rmod.result})'
                        assert cond_zero, f'TEST FAILED zero dut({response_dut.zero}), rmod({response_rmod.zero})'
    ...
    def write(self, t):
        if(isinstance(t, uvm_sequence_item)):
            ...
            if self.tr.get_transaction().request.reset == 0:
