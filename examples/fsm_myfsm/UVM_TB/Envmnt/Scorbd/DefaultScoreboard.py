...
class DefaultScoreboard(uvm_scoreboard):
    ...
    def check_phase(self):
        ...
        while self.dut_result_get_port.can_get() and self.refmodel_result_get_port.can_get():
            ...
            if not success_dut or not success_rmod:
                ...
                if request_dut.i_rst_n == 1 and self.enable_scoreboarding == False:
                ...
                if self.enable_scoreboarding:  
                    try:
                        ...
                        ## Save conditions
                        result_equal = response_dut.o_y == response_rmod.o_y
                        final_estate = response_dut.o_state == response_rmod.o_state

                        # Make assertions
                        #print(f'{request_dut.i_x}: o_y dut({hex(response_dut.o_y)}), rmod({hex(response_rmod.o_y)})')
                        #print(f'{request_dut.i_x}: o_state dut({hex(response_dut.o_state)}), rmod({hex(response_rmod.o_state)})')
                        assert result_equal, f'TEST FAILED o_y dut({hex(response_dut.o_y)}), rmod({hex(response_rmod.o_y)})'
                        assert final_estate, f'TEST FAILED o_state dut({hex(response_dut.o_state)}), rmod({hex(response_rmod.o_state)})'


                        # Save on report file if necessary (watch Misces/UVMEnvReport.py for help)
                        """if result_equal and final_estate:
    ...
    def write(self, t):
        if(isinstance(t, uvm_sequence_item)):
            ...
            if self.tr.get_transaction().request.i_rst_n == 1:
